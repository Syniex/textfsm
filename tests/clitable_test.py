...
from textfsm import clitable, copyable_regex_object
...
class UnitTestIndexTable(unittest.TestCase):
    ...
    def testParseIndex(self):
        ...
        for col in ("Command", "Vendor", "Template", "Hostname"):
            self.assertTrue(
                isinstance(
                    indx.compiled[1][col], copyable_regex_object.CopyableRegexObject
                )
            )
        ...
        self.assertTrue(
            isinstance(
                indx.compiled[1]["Hostname"], copyable_regex_object.CopyableRegexObject
            )
        )
    ...
    def testGetRowMatch(self):
        ...
        self.assertEqual(2, indx.GetRowMatch({"Hostname": "abc", "Vendor": "VendorB"}))
...
class UnitTestCliTable(unittest.TestCase):
    ...
    def testCompletion(self):
        ...
        self.assertEqual(
            "a(b(c)?)?", re.sub(r"(\[\[.+?\]\])", indx._Completion, "a[[bc]]")
        )
        self.assertEqual(
            "a(b(c)?)? de(f)?",
            re.sub(r"(\[\[.+?\]\])", indx._Completion, "a[[bc]] de[[f]]"),
        )
    ...
    def testCliCompile(self):
        ...
        self.assertEqual(
            "sh(o(w)?)? ve(r(s(i(o(n)?)?)?)?)?", self.clitable.index.index[1]["Command"]
        )
        self.assertEqual(None, self.clitable.index.compiled[1]["Template"])
        self.assertTrue(self.clitable.index.compiled[1]["Command"].match("sho vers"))

    def testParseCmdItem(self):
        """Tests parsing data with a single specific template."""
        t = self.clitable._ParseCmdItem(
            self.input_data, template_file=self.template_file
        )
    ...
    def testParseCmd(self):
        ...
        self.assertEqual(self.clitable.table, "Col1, Col2, Col3\na, b, c\nd, e, f\n")

    def testParseWithTemplate(self):
        """Tests parsing with an explicitly declared the template."""
        self.clitable.ParseCmd(
            self.input_data,
            attributes={"Command": "sh vers"},
            templates="clitable_templateB",
        )
        self.assertEqual(self.clitable.table, "Col1, Col4\na, b\nd, e\n")

    def testParseCmdFromIndex(self):
        """Tests parsing with a template found in the index."""
        self.clitable.ParseCmd(
            self.input_data, attributes={"Command": "sh vers", "Vendor": "VendorB"}
        )
        self.assertEqual(self.clitable.table, "Col1, Col2, Col3\na, b, c\n")
        self.clitable.ParseCmd(
            self.input_data, attributes={"Command": "sh int", "Vendor": "VendorA"}
        )
        self.assertEqual(self.clitable.table, "Col1, Col2, Col3\nd, e, f\n")
        ...
        self.assertRaises(
            ...
            self.clitable.ParseCmd,
            self.input_data,
            attributes={"Command": "show vers", "Vendor": "bogus"},
        )
        self.assertRaises(
            clitable.CliTableError,
            self.clitable.ParseCmd,
            self.input_data,
            attributes={"Command": "unknown command", "Vendor": "VendorA"},
        )

    def testParseWithMultiTemplates(self):
        """Tests that multiple matching templates extend the table."""
        self.clitable.ParseCmd(
            self.input_data, attributes={"Command": "sh ver", "Vendor": "VendorA"}
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.clitable.table, "Col1, Col4, Col2, Col3\na, b, b, c\nd, e, e, f\n"
        )
        self.assertRaises(
            IOError,
            self.clitable.ParseCmd,
    ...
    def testRequireCols(self):
        ...
        self.assertRaises(
            clitable.CliTableError, clitable.CliTable, "nondefault_index", "testdata"
        )
    ...
    def testTableSort(self):
        ...
        input_data2 = "a e c\n" "d b f\n"
        self.clitable.ParseCmd(
            self.input_data + input_data2, attributes={"Command": "sh ver"}
        )
        self.assertEqual(
            self.clitable.table,
            "Col1, Col2, Col3\na, b, c\nd, e, f\na, e, c\nd, b, f\n",
