...
class TextFSMValue(object):
    ...
    def Parse(self, value):
        ...
        if not re.match(r"^\(.*\)$", self.regex) or regex_without_brackets.count(
            "("
        ) != regex_without_brackets.count(")"):
            raise TextFSMTemplateError(
...
class TextFSMRule(object):
    ...
    def __init__(self, line, line_num=-1, var_map=None):
        ...
        line = line.strip()
        if not line:
            raise TextFSMTemplateError("Null data in FSMRule. Line: %s" % self.line_num)
