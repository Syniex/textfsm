...
from textfsm import terminal, texttable
...
class UnitTestRow(unittest.TestCase):
    ...
    def testSetValues(self):
        ...
        self.assertRaises(
            TypeError, self.row._SetValues, ["seven", "eight", "nine", "ten"]
        )
...
class UnitTestTextTable(unittest.TestCase):
    ...
    def testFilter(self):
        ...
        filtered_table = old_table.Filter(function=lambda row: row["a"] == "10")
    ...
    def testMap(self):
        old_table = self.BasicTable()
        filtered_table = old_table.Map(function=lambda row: row["a"] == "10" and row)
        self.assertEqual(1, filtered_table.size)
...
"""
    ...
    def testAppend(self):
        ...
        self.assertRaises(TypeError, t.Append, {"a": "11", "b": "12", "d": "13"})
    ...
    def testAddColumn(self):
        ...
        self.assertEqual(["a", "Wine", "b", "Spirits", "c", "Beer"], t.header.values)
        self.assertEqual(["10", "Merlot", "20", "", "30", ""], t[2].values)
    ...
    def testFormattedTableColor(self):
        ...
        for color_key in terminal.FG_COLOR_WORDS:
            ...
            t.FormattedTable()
            self.assertEqual(
                sorted(t[0].color), sorted(terminal.FG_COLOR_WORDS[color_key])
            )
        for color_key in terminal.BG_COLOR_WORDS:
            t[0].color = terminal.BG_COLOR_WORDS[color_key]
            t.FormattedTable()
            self.assertEqual(
                sorted(t[0].color), sorted(terminal.BG_COLOR_WORDS[color_key])
            )
    ...
    def testFormattedTable2(self):
        ...
        t.Append(
            ("DeviceA", "lo0.0", "up", "up", "inet", ["127.0.0.1", "10.100.100.1"])
        )
